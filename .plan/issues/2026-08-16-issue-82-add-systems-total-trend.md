...
- [ ] **Step 1: Implementation**
  ...
  - 시계열 차트에 짝이 되는 이름을 겹쳐 그린다. `Frame`을 보면 합계가, 합계를 보면 `Frame`이 같은
    축에 함께 나온다. 따로 보면 "작업량이 늘어 프레임이 밀리기 시작했다"를 읽을 수 없기 때문이다.
    두 시계열의 구간이 다를 수 있으므로 축은 합집합으로 만들고 없는 구간은 값을 비운다.
- [ ] **Step 2: Tests**
